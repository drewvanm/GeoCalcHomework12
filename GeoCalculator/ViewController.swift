...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var tempP1: GeoCalcLabel!
    @IBOutlet weak var tempP2: GeoCalcLabel!
    @IBOutlet weak var weatherP1: GeoCalcLabel!
    @IBOutlet weak var weatherP2: GeoCalcLabel!
    //end of weather
    ...
    func clearWeatherViews(){
        self.imageP1.image = nil
        self.imageP2.image = nil
        self.weatherP1.text = ""
        self.weatherP2.text = ""
        self.tempP1.text = ""
        self.tempP2.text = ""
    ...
    func doCalculatations()
    {
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p1lt, p1ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.tempP1.text = "\(w.temperature.roundTo(places: 1))°"
                    self.imageP1.image = UIImage(named: w.iconName)
                    self.weatherP1.text = w.summary
                }
            }
        }
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p2lt, p2ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.tempP2.text = "\(w.temperature.roundTo(places: 1))°"
                    self.imageP2.image = UIImage(named: w.iconName)
                    self.weatherP2.text = w.summary
                }
            }
        }
